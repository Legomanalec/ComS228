...
public class Plot {
	...
	private static class MyPanel extends JPanel {
		...
		private void drawPoint(Graphics2D g2, Point p, Color color) {
			...
			g2.setPaint(color);
			g2.drawOval(px, py, POINT_WIDTH, POINT_WIDTH);// Ovals look better
			g2.fillOval(px, py, POINT_WIDTH, POINT_WIDTH);// fill the Oval
		 ...
		 * @param y2
		 */
		private void drawSegment(Graphics2D g2, Segment s, Color color) {
			g2.setPaint(color);
			Point p = s.getP();
			Point q = s.getQ();
			
			//stops the plot from drawing duplicate segments
			if(p.compareTo(q) == 0)
				return;
		...
		public void paint(Graphics g) {
			...
			//I initially had one for loop for drawing segments, but the orange segments kept overlapping blue ones
			//so I just made sure oranges were plotted first then blue could overlap them
			
			//Comparator segments
			for(int i = 0; i < segments.length; i++) {
				
				if(segments[i].getQ().equals(RotationalPointScanner.plotMedPoint) 
						|| segments[i].getP().equals(RotationalPointScanner.plotMedPoint))
					drawSegment(g2, segments[i], Color.ORANGE);
			}
			//MedianCoordinatePoint segments
			for (int i = 0; i < segments.length; i++) {
				
				if(!segments[i].getQ().equals(RotationalPointScanner.plotMedPoint) 
						&& !segments[i].getP().equals(RotationalPointScanner.plotMedPoint))
					drawSegment(g2, segments[i], Color.BLUE);
			}
			
			
			//Plots the MedianCoordinatePoint as green 
			drawPoint(g2, RotationalPointScanner.plotMedPoint, Color.BLUE);
			
			//moved this statement down because the lines would sometimes cover the points
			for (int i = 0; i < points.length; i++) {
				drawPoint(g2, points[i], Color.RED);
			}
		}
