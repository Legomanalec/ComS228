...
public class PolarAngleComparator implements Comparator<Point>
{
    ...
    {
    	if(p1.equals(p2) 
    		|| ((!p1.equals(referencePoint) && !p2.equals(referencePoint)) && crossProduct(p1, p2) == 0 && dotProduct(p1, p2) > 0))
    		return 0;
    	else if(p1.equals(referencePoint) //p1 == referencePoint
    			//1
    			|| (!p2.equals(referencePoint) && ((p1.getY() < referencePoint.getY() 
    					&& p2.getY() < referencePoint.getY() && crossProduct(p1, p2) > 0
    			//2
    			|| (p1.getY() == referencePoint.getY() 
    				&& (p2.getY() < referencePoint.getY() //a
    						|| (p2.getY() == referencePoint.getY() && p1.getX() > referencePoint.getX() && p2.getX() < referencePoint.getX())//b
    							|| p2.getY() > referencePoint.getY() && p1.getX() > referencePoint.getX()))//c
    			//3
    			|| p1.getY() > referencePoint.getY() 
    				&& ((p2.getY() > referencePoint.getY() && crossProduct(p1, p2) > 0) //a
    						|| (p2.getY() == referencePoint.getY() && p2.getX() < referencePoint.getX()) //b
    							|| p2.getY() < referencePoint.getY())))))//c
    		return -1;
    	else
    		return 1;
    }
    ...
    public int compareDistance(Point p1, Point p2)
    {
    	...
    	if(dotProduct(p1, p1) == dotProduct(p2, p2))
    		return 0;
    	else if(dotProduct(p1, p1) < dotProduct(p2, p2)) //dot product of something by the same thing is square of magnitude
    		return -1;
    	return 1;
    }
